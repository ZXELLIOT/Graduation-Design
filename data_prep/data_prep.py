...
def clean_text(text: str) -> Optional[str]:
    ...
    清洗规则:
        ...
        3. 剔除过长文本 —— 超出训练 max_length 的文本会被截断，影响质量
    参数:
        text: 待清洗的原始字符串。
    返回:
...
def write_csv(
    ...
    with open(output_csv_path, "w", encoding="utf-8-sig", newline="") as f:
        ...
        if with_negative:
            ...
            for i in tqdm(
                range(n),
                desc=f"{stage_name} 生成训练CSV ({os.path.basename(output_csv_path)})",
                unit="条",
                dynamic_ncols=True,
            ):
            ...
            for query, response in tqdm(
                pairs,
                desc=f"{stage_name} 生成语料CSV ({os.path.basename(output_csv_path)})",
                unit="条",
                dynamic_ncols=True,
            ):
                writer.writerow([query, response])
...
def build_standard_datasets(
    ...
    # 步骤1：从 JSON 提取并清洗问答对
    train_pairs = load_first_pairs_from_json(train_json_path, "train")
    valid_pairs = load_first_pairs_from_json(valid_json_path, "valid")
    test_pairs = load_first_pairs_from_json(test_json_path, "test")
    large_pairs = load_first_pairs_from_json(large_json_path, "large")
    # 步骤2：写出 CSV（不同随机种子保证各集合负采样独立）
    write_csv(train_pairs, output_train_neg_csv, with_negative=True, seed=seed, stage_name="train")
    write_csv(valid_pairs, output_valid_neg_csv, with_negative=True, seed=seed + 1, stage_name="valid")
    write_csv(test_pairs, output_test_neg_csv, with_negative=True, seed=seed + 2, stage_name="test")
    write_csv(large_pairs, output_large_csv, with_negative=False, stage_name="large")
...
def main():
    ...
    print("=" * 60)
    print("处理完成，输出汇总如下")
    print(f"训练集: {TRAIN_NEG_CSV_PATH} ({train_count:,} 对)")
    print(f"验证集: {VALID_NEG_CSV_PATH} ({valid_count:,} 对)")
    print(f"测试集: {TEST_NEG_CSV_PATH} ({test_count:,} 对)")
    print(f"语料库: {LARGE_CSV_PATH} ({large_count:,} 对)")
    print("=" * 60)
